...
def main() -> int:
    ...
    try:
        ...
        jira_progress_reporter.publish_stage(
            "workflow_triggered",
            {"detail": "Deployment workflow accepted the Jira ticket and started processing."},
        )
        ...
        jira_progress_reporter.publish_stage(
            "target_resolved",
            {
                ...
                "detail": "Resolved the GitOps path and ArgoCD application for the request.",
            },
        )
        ...
        jira_progress_reporter.publish_stage(
            "lock_acquired",
            {
                ...
                "detail": "Deployment lock acquired successfully.",
            },
        )
            ...
            if current_tag == desired_version:
                jira_progress_reporter.publish_stage(
                    "gitops_commit_pushed",
                    {
                        ...
                        "detail": "No new GitOps commit was needed; verifying the current deployed revision.",
                    },
                )
                if test_mode:
                    ...
                    jira_progress_reporter.publish_stage(
                        "argocd_sync_started",
                        {
                            ...
                            "detail": "Test mode reconciliation started against the simulated ArgoCD state.",
                        },
                    )
                    ...
                    jira_progress_reporter.publish_stage(
                        "argocd_sync_started",
                        {
                            "target": target,
                            "gitops_commit": current_revision,
                            "detail": "Waiting for ArgoCD to verify the current revision.",
                        },
                    )
                ...
                jira_progress_reporter.publish_stage(
                    "gitops_commit_pushed",
                    {
                        ...
                        "resolved_version": desired_version,
                        "gitops_commit": gitops_commit,
                        "detail": "GitOps values file was updated and pushed successfully.",
                    },
                )
                try:
                    if test_mode:
                        ...
                        jira_progress_reporter.publish_stage(
                            "argocd_sync_started",
                            {
                                "target": target,
                                "gitops_commit": gitops_commit,
                                "detail": "GitOps commit pushed. Waiting for ArgoCD to reconcile it.",
                            },
                        )
                        if args.trigger_argocd_sync:
                            argocd.sync_app(target["argocd_app"])
        ...
        jira_progress_reporter.publish_stage(
            "argocd_synced_healthy",
            {
                "target": target,
                "gitops_commit": gitops_commit,
                "detail": "ArgoCD reported the expected revision as Synced and Healthy.",
            },
        )
        ...
        jira_progress_reporter.publish_stage(
            "post_checks_completed",
            {
                "target": target,
                "gitops_commit": gitops_commit,
                "detail": "Post-deployment checks completed.",
            },
        )
